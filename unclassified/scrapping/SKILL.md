...
- **"Get an Instagram reel"** → See `references/instagram-browser-automation.md` for the browser-automation method (Playwright) used when the API is blocked or rate-limited

When in doubt, check the platform reference file for the full endpoint list.

## Instagram Browser Automation Fallback

When the ScrapeCreators API key is unavailable or the target platform blocks API access, use the browser automation fallback for Instagram reels.

### When to use the fallback

- No `SCRAPECREATORS_API_KEY` in environment
- Instagram blocks ScrapeCreators requests (anti-bot detection)
- Need full caption text that API endpoints truncate
- Need screenshot for vision analysis

### Fallback script

```bash
# Extract reel metadata via Playwright browser automation
python ~/.hermes/skills/scraping-intel/scripts/ig_reel_extract.py "https://www.instagram.com/reel/SHORTCODE/"
```

**What it does:**
1. Launches headless Chromium with anti-detection args
2. Navigates to reel URL, waits for DOM load
3. Dismisses Instagram signup modal (Escape key + Close button click)
4. Extracts all visible text elements from the page
5. Classifies text into: caption candidates, hashtags, comments, meta
6. Saves screenshot to `/tmp/ig_reel_{shortcode}.png`
7. Outputs JSON with structured data

**Output format:**
```json
{
  "shortcode": "DZCoYFBRFIH",
  "url": "https://www.instagram.com/reel/DZCoYFBRFIH/",
  "screenshot": "/tmp/ig_reel_DZCoYFBRFIH.png",
  "caption_candidates": ["Creator handle", "Full caption text..."],
  "hashtags": ["#tag1", "#tag2"],
  "comments_top": ["comment text..."],
  "meta": ["likes count", "follow button text..."],
  "raw_texts_count": 102
}
```

**Requirements:** `playwright` Python package installed. Install with: `pip install playwright` then `playwright install chromium`.

### Workflow pitfall: Check existing scripts first

**Always search existing agent scripts before building a new solution.** In this environment, the `hive` profile has `ig_reel_extract.py` as a working script. Rather than trying urllib/firecrawl/ScrapeCreators and failing, invoke the existing script directly or copy its pattern.

```bash
# Check if any agent already has a script for the platform
