...
Hermes already has `delegate_task` for parallel subagent work. Use dmux for:
- **Claude Code desktop sessions** that need human interaction
- **Long-running processes** you want to monitor in real-time
- **Cross-tool work** (Hermes + Claude Code + Codex simultaneously)
...
| **Hermes `delegate_task`** | In-process subagent spawning | Programmatic parallelism within a session |
| **Hermes `cronjob`** | Scheduled parallel work | Batch background jobs |
...
- `subagent-driven-development` — for Hermes-native parallel subagent work
- `swarm-execution-patterns` — for Edgeless swarm orchestration
- `claude-code` — for Claude Code desktop sessions
