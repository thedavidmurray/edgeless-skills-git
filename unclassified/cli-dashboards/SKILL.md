---
name: cli-dashboards
title: CLI Dashboards and Terminal Reporting
category: software-development
description: Build Tufte-minimal data dashboards in the terminal using rich + sqlite. No web stack, no polling until design is locked. Simulated backfill for preview.
---
...
## Resilient Data Sourcing for Agent Infrastructure

When building dashboards that consume agent infrastructure (Hermes cron, Paperclip, ChromaDB, vault), APIs are often slow or unreliable. Use the patterns in `references/agent-dashboard-resilient-sourcing.md`:

- **Hermes cron** — read `~/.hermes/cron/jobs.json` directly; never parse `hermes cron list` output
- **Paperclip backlog** — when the Issues API is slow, fall back to parsing the `fleet-daily` markdown report (`claude-vault/13-Reports/daily-alignment/YYYY-MM-DD-fleet-daily.md`)
- **Service health** — probe with `curl --max-time 2` instead of inferring from tool availability
- **Vault metrics** — use `find` + `du` for large trees; don't attempt recursive Python walks on 4.9 GB vaults
- **Graceful degradation** — always render something; label the data source; separate fetch from render

## References

- See `references/rune-flux-report.md` for the trading-specific instance of the terminal dashboard pattern (2026-05-21).
- `references/agent-dashboard-resilient-sourcing.md` — Resilient patterns for sourcing data from Hermes cron, Paperclip, ChromaDB, and vault for agent infrastructure dashboards (2026-06-03).
- `references/edgeless-os-dashboard.md` — Swarm infrastructure dashboard instance: 20 agents, Paperclip backlog, cron health, vault metrics (2026-06-04).
