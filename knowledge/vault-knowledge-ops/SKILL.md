...
description: >
  Manage the knowledge vault end-to-end: enforce canonical taxonomy and structure,
  enrich incoming content with multi-track semantic classification, and run
  automated synthesis agents that keep the vault self-organizing. Covers vault
  structure enforcement, content ingestion & routing, collision detection,
  migration workflows, and scheduled compilation.
version: 1.0.0
author: Hermes Agent
license: MIT
metadata:
  tags: [vault, taxonomy, knowledge-management, content-enrichment, chromadb, routing, classification]
  tier: task-specific
  domain: knowledge
  color: blue
prerequisites:
  commands: [python3, grep]
---
...
| Swarm KB | `~/claude-projects/claude-vault/03-Knowledge/Swarm/` | `*/swarm/` (outside vault) ❌ |
...
### Source-Specific Enrichment: arXiv and Academic Papers

When enriching a single arXiv item from the RSS pipeline (e.g., EDGA-6025), the minimal note produced by the triage pipeline contains only title, abstract, and score. Upgrading it to a full KB article requires fetching structured metadata and expanding the content.

**arXiv API (metadata only):**
```bash
# Fetch abstract, authors, published date, categories
curl -s "https://export.arxiv.org/api/query?id_list=2605.27681" | grep -o '<summary>[^<]*' | sed 's/<summary>//'
curl -s "https://export.arxiv.org/api/query?id_list=2605.27681" | grep -o '<name>[^<]*' | sed 's/<name>//'
curl -s "https://export.arxiv.org/api/query?id_list=2605.27681" | grep -o '<published>[^<]*' | sed 's/<published>//'
curl -s "https://export.arxiv.org/api/query?id_list=2605.27681" | grep -o '<category term="[^\"]*' | sed 's/<category term="//'
```

**Jina AI reader (quick webpage extraction):**
```bash
# Extract readable markdown from any URL when an API is unavailable
curl -s "https://r.jina.ai/http://arxiv.org/abs/2605.27681" | head -100
```

**Limitations:** Jina AI returns HTML-page content only; it does NOT extract from PDF URLs. For direct arXiv PDF URLs, see `references/arxiv-enrichment-patterns.md` §"PDF Direct Extraction" for the curl+pdftotext fallback chain.

**Two-stage enrichment flow:**
1. **RSS pipeline** creates minimal note: `claude-vault/03-Knowledge/RSS/cs.AI/reasoning-and-planning-with-dynamically-.md` (title, abstract, score, source)
2. **Enrichment agent** (this skill) upgrades it to a full KB article with: executive summary, technical deep-dive, comparison tables, real-world applications, action items, key quotes, cross-references, and full frontmatter (`kb_score`, `enrichment_tier`, `vault_connections`)

See `references/arxiv-enrichment-patterns.md` for full reproduction commands and a worked example.
...
- **Paperclip checkout `expectedStatuses` enum**: The `POST /api/issues/{id}/checkout` endpoint requires `expectedStatuses` as an array of valid enum values: `backlog`, `todo`, `in_progress`, `in_review`, `done`, `blocked`, `cancelled`. `open` is NOT a valid enum and will fail validation. The correct call for a freshly-created ticket is `{"agentId": "...", "expectedStatuses": ["todo"]}`.
- **Paperclip comment API uses `body` not `content`**: When posting a comment to an issue via `POST /api/issues/{id}/comments`, the JSON field is `body`, not `content`. Passing `{"content": "..."}` will silently create a comment with empty text.
- **Vault `_system/` directory may not exist**: The canonical vault at `~/claude-vault/` (or `~/claude-projects/claude-vault/`) may be sparse. In practice, only `00-Inbox/`, `03-Knowledge/`, and `13-Reports/` exist. There is no `_system/` directory, no `templates/` directory, and no `TAXONOMY.md` file. Do not assume these exist when creating new notes. Create directories as needed.
- **Codex-vault/ is a rogue duplicate — NEVER write there**: The canonical vault is `claude-vault/` only. `Codex-vault/` is a deprecated duplicate that has been used incorrectly in some sessions. If you find yourself writing to `Codex-vault/`, stop and migrate to `claude-vault/` immediately. The `claude-vault/` directory already exists under `~/claude-projects/` and is the single source of truth.
- **Paperclip-first task tracking**: Do NOT create markdown task files in `07-Tasks/` or any vault subdirectory. All tasks must be filed as Paperclip issues under the appropriate project (e.g., "Knowledge Spine"). The `07-Tasks/` convention is obsolete. After creating a Paperclip issue, delete any markdown task file you created — tasks live in Paperclip, not in the vault.
- **Never create new top-level directories in claude-projects**: If a canonical location seems missing, ask in `#bot-backroom` before writing. The directory policy is enforced by `DIRECTORY-POLICY.md` at the project root. Read it before creating directories or cloning repos.
- **`web_extract` truncates arXiv PDFs** — PDF URLs passed to web_extract return only ~5K chars (the first section of the paper). Do NOT retry with different parameters; immediately fall through to curl + pdftotext. See `references/arxiv-enrichment-patterns.md` §"PDF Direct Extraction".
...
│   ├── arxiv-enrichment-patterns.md       # arXiv API metadata extraction, Jina AI reader, PDF direct extraction
│   └── academic-pdf-knowledge-spine-ingestion.md  # Full textbook ingestion workflow: PDF → knowledge spine → Paperclip issues → swarm dispatch
├── templates/
...
- `references/vault-taxonomy/collision-remediation-session-2026-05-06.md` — Session recipes for specific collision types
...
- `references/arxiv-enrichment-patterns.md` — arXiv API metadata extraction, Jina AI reader, PDF direct extraction (curl+pdftotext fallback), two-stage RSS→KB enrichment flow. Now covers both RSS-pipeline and user-initiated direct PDF workflows.
- `references/academic-pdf-knowledge-spine-ingestion.md` — Full textbook ingestion workflow: PDF download → text extraction → knowledge spine structuring → Paperclip issue creation → swarm dispatch. Includes SLP3 battle-tested templates, batch groupings, tracker format, and canonical location enforcement.
